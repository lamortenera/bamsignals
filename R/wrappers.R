...
#' @return \itemize{
#'   \item \code{pileup} and \code{depth}: a CountSignals object with a signal 
#'     per region
#'   \item \code{count}: a vector with one element per region or, 
...
#' \code{pileup}: for each base pair in the ranges, compute the number of reads
