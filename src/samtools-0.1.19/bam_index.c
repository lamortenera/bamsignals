...
static void fill_missing(bam_index_t *idx)
{
	...
	for (i = 0; i < idx->n; ++i) {
		...
		//that's really retarded... who wrote that?
		//they should have looped backwards...
		for (j = 1; j < idx2->n; ++j)
			if (idx2->offset[j] == 0)
...
// modified by me
bam_iter_t bam_iter_query_new(const bam_index_t *idx, int tid, int beg, int end)
{
	uint16_t *bins;
	int i, n_bins, n_off;
	pair64_t *off;
	...
	for (i = 0; i <= n_bins; ++i){
		uint16_t binsi;
		if (i == n_bins){ binsi = bins[i-1] + 1; }
		else { binsi = bins[i]; }
		printf("bin %i ;", binsi);
		
		if ((k = kh_get(i, index, binsi)) != kh_end(index)){
			bam_binlist_t *p = &kh_value(index, k);
			int j;
			for (j = 0; j < p->n; ++j){
				int block_offset;
				int64_t block_address;
				int64_t pos = p->list[j].u;
				block_offset = pos & 0xFFFF;
				block_address = pos >> 16;
				printf("%i,%i-", block_address, block_offset); 
				pos = p->list[j].v;
				block_offset = pos & 0xFFFF;
				block_address = pos >> 16;
				printf("%i,%i:",block_address, block_offset); 
	...
	//determining min_off and max_off
	if (idx->index2[tid].n > 0) {
		min_off = (beg>>BAM_LIDX_SHIFT >= idx->index2[tid].n)? idx->index2[tid].offset[idx->index2[tid].n-1]
			: idx->index2[tid].offset[beg>>BAM_LIDX_SHIFT];
		max_off = ((end>>BAM_LIDX_SHIFT) + 1 >= idx->index2[tid].n)? UINT64_MAX
			: idx->index2[tid].offset[(end>>BAM_LIDX_SHIFT) + 1];
	...
	} else {// tabix 0.1.2 may produce such index files
		min_off = 0; 
		max_off = UINT64_MAX;
	}
	{
		printf("relbin1-relbin2: %i-%i\n",4681 + (beg>>BAM_LIDX_SHIFT), 4681 + (end>>BAM_LIDX_SHIFT) + 1); 
		int block_offset;
		int64_t block_address;
		int64_t pos = min_off;
		block_offset = pos & 0xFFFF;
		block_address = pos >> 16;
		printf("min_off-max_off:%i,%i-", block_address, block_offset); 
		pos = max_off;
		block_offset = pos & 0xFFFF;
		block_address = pos >> 16;
	...
	for (i = n_off = 0; i < n_bins; ++i) {
		if ((k = kh_get(i, index, bins[i])) != kh_end(index)) {
			...
			bam_binlist_t *p = &kh_value(index, k);
			for (j = 0; j < p->n; ++j)
				if (p->list[j].v > min_off && p->list[j].u <= max_off) off[n_off++] = p->list[j];
		}
	}
	free(bins);
	if (n_off == 0) {
