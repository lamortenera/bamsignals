...
All bamsignal count, profile and coverage methods discussed above support dealing with paired end sequencing data. With `paired.end=TRUE`, only the first read in a properly aligned pair are considered. This avoids counting both reads in read pair which may bias downstream analysis. For `bamCount()` and `bamProfile()` exclusively, there exists another flag called `paired.end.midpoint`. The actual length of a fragment is stored in the TLEN field of a paired end bam file. Setting `paired.end.midpoint=TRUE` shifts the counting position from 5'end to the midpoint of the physical fragment.
