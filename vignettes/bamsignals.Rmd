...
The goal of the `bamsignals` package is to load count data from bam files as easily and quickly as possible. A typical workflow without the `bamsignals` package requires to firstly load all reads in R (_e.g._ using the `Rsamtools` package), secondly process them and lastly convert them into counts. The `bamsignals` package optimizes this workflow by merging these steps into one using efficient C code, which makes the whole process easier and faster. Additionally, `bamsignals` inherently can deal with paired end data which is a problem in most workflows.
...
## Read profiles for each region with `bamcountprofile()`

If you are interested in counting how many reads map to each base pair of your genes, the `bamcountprofile()` function might save you a day.
```{r}
...
### Counting on each strand separately
As for the `bamcount()` function, the reads can be counted for each strand separately
```{r}
...
All bamsignal count, profile and coverage methods discussed above support dealing with paired end sequencing data. With `paired.end=TRUE`, only the first read in a properly aligned pair are considered. This avoids counting both reads in read pair which may bias downstream analysis. For `bamcount()` and `bamcountprofile()` exclusively, there exists another flag called `paired.end.midpoint`. The actual length of a fragment is stored in the TLEN field of a paired end bam file. Setting `paired.end.midpoint=TRUE` shifts the counting position from 5'end to the midpoint of the physical fragment.

## Exclude ambiguous reads with the `mapq` argument

Most mapping software (_e.g._ bowtie2) stores information about mapping confidence in the MAPQ field of a bam file. In general, it is recommended to exclude reads with bad mapping quality because their mapping location is ambiguous. In bowtie2, a mapping quality of 20 or less indicates that there is at least a 1 in 20 chance that the read truly originated elsewhere. In that case, the `mapq` argument is helpful.
